...
pub fn check_device_posture(device_id: &str) -> Vec<DevicePosture> {
    ...
    // 1. ADB activé
    if let Some(val) = adb_device(device_id, &["shell", "settings", "get", "global", "adb_enabled"]) {
    ...
    if let Some(val) = adb_device(device_id, &["shell", "settings", "get", "global", "package_verifier_enable"]) {
        ...
        checks.push(DevicePosture {
            ...
            value: trimmed.to_string(),
            status: if trimmed == "1" { PostureStatus::Good } else { PostureStatus::Bad },
            fix_command: Some("settings put global package_verifier_enable 1".into()),
        });
    }

    // 5. Vérification ADB
    if let Some(val) = adb_device(device_id, &["shell", "settings", "get", "global", "verifier_verify_adb_installs"]) {
        let trimmed = val.trim();
        checks.push(DevicePosture {
            name: "Vérification ADB".into(),
            value: trimmed.to_string(),
            status: if trimmed == "0" { PostureStatus::Bad } else { PostureStatus::Good },
    ...
    if let Some(val) = adb_device(device_id, &["shell", "settings", "get", "secure", "enabled_accessibility_services"]) {
        ...
        checks.push(DevicePosture {
            ...
            value: if is_empty { "Aucun".into() } else { trimmed.to_string() },
            status: if is_empty { PostureStatus::Good } else { PostureStatus::Bad },
            fix_command: None,
        });
    }

    // 7. Clavier par défaut
    if let Some(val) = adb_device(device_id, &["shell", "settings", "get", "secure", "default_input_method"]) {
        checks.push(DevicePosture {
    ...
    if let Some(val) = adb_device(device_id, &["shell", "settings", "get", "secure", "lockscreen.password_type"]) {
        let trimmed = val.trim();
        let status = trimmed
            .parse::<i64>()
            .map(|v| if v >= 65536 { PostureStatus::Good } else { PostureStatus::Bad })
    ...
    // 9. Mode localisation
    if let Some(val) = adb_device(device_id, &["shell", "settings", "get", "secure", "location_mode"]) {
        checks.push(DevicePosture {

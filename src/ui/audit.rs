...
pub fn draw_audit(app: &mut PhoneTvApp, ui: &mut egui::Ui, ctx: &egui::Context) {
    ...
    let device_name = app.get_selected()
        .map(|d| d.name.clone())
                ...
                .show(ui, |ui| {
                    ...
                    if let Some(device) = app.get_selected() {
                        ui.horizontal(|ui| {
                            ...
                            ui.label(egui::RichText::new(format!("{} {}", icon, device.name)).size(14.0).strong());
                            ...
                            ui.label(egui::RichText::new(&device.status).size(12.0).color(theme::text_secondary(dark)));
                        });
                        ui.add_space(8.0);
                    }
                    ...
                    ui.add(egui::TextEdit::singleline(&mut app.settings.openrouter_api_key)
                        .password(true)
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if ui.add_enabled(
                            can_test,
                            egui::Button::new(egui::RichText::new("Tester le modele").color(egui::Color32::WHITE))
                            ...
                            std::thread::spawn(move || {
                                ...
                                let _ = tx.send(BgEvent::LlmModelValid { valid, model, error });
